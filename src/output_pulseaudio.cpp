...
struct t_samplespec {
	...
	{
		PFC_ASSERT(is_valid());
		return (t_size)audio_math::time_to_samples(p_time, m_sample_rate);
	}
	double samples_to_time(t_size p_samples) const
	{
		PFC_ASSERT(is_valid());
		return audio_math::samples_to_time(p_samples, m_sample_rate);
	}
	...
	{
		return m_sample_rate == p_spec2.m_sample_rate && m_channels == p_spec2.m_channels &&
		       m_channel_config == p_spec2.m_channel_config;
	}

	inline bool operator!=(const t_samplespec &p_spec2) const { return !(*this == p_spec2); }
...
static const GUID guid_cfg_pulseaudio_fade_out_track = {
    0xe136e959, 0x929b, 0x4005, {0xaa, 0x9e, 0x8e, 0x8b, 0x91, 0x5b, 0x5d, 0x2}
};
...
static const GUID guid_cfg_pulseaudio_prebuffer = {
    0x64cd1e28, 0x87ea, 0x41e5, {0xaf, 0x3d, 0xc6, 0xcd, 0x2f, 0x52, 0xac, 0xee}
};

static advconfig_branch_factory g_pulseaudio_output_branch("PulseAudio output", guid_cfg_pulseaudio_branch,
                                                           advconfig_branch::guid_branch_playback, 0);
static advconfig_integer_factory cfg_pulseaudio_seek_fade_out("Fade out on seek (milliseconds)",
                                                              guid_cfg_pulseaudio_fade_out_seek,
                                                              guid_cfg_pulseaudio_branch, 0, 15, 0, 1000, 0);
static advconfig_integer_factory cfg_pulseaudio_seek_fade_in("Fade in on seek (milliseconds)",
                                                             guid_cfg_pulseaudio_fade_in_seek,
                                                             guid_cfg_pulseaudio_branch, 0, 15, 0, 1000, 0);
static advconfig_integer_factory cfg_pulseaudio_track_fade_out("Fade out on manual track change (milliseconds)",
                                                               guid_cfg_pulseaudio_fade_out_track,
                                                               guid_cfg_pulseaudio_branch, 0, 0, 0, 1000, 0);
static advconfig_integer_factory cfg_pulseaudio_track_fade_in("Fade in on manual track change (milliseconds)",
                                                              guid_cfg_pulseaudio_fade_in_track,
                                                              guid_cfg_pulseaudio_branch, 0, 0, 0, 1000, 0);
static advconfig_checkbox_factory cfg_pulseaudio_minreq_workaround("Enable workaround for driver issue",
                                                                   guid_cfg_pulseaudio_minreq_workaround,
                                                                   guid_cfg_pulseaudio_branch, 0, false);
static advconfig_integer_factory cfg_pulseaudio_prebuf("Request prebuffer (milliseconds)",
                                                       guid_cfg_pulseaudio_prebuffer, guid_cfg_pulseaudio_branch, 0,
                                                       200, 0, 100000, 0);
...
public:
	...
	output_pulse(const GUID &p_device, double p_buffer_length, bool p_dither, t_uint32 p_bitdepth)
	    : buffer_length(p_buffer_length), m_incoming_ptr(0), progressing(false), draining(false), drained(false),
	      ...
	      cfg_track_fade_in((size_t)cfg_pulseaudio_track_fade_in), fade_in_next_ms(0), active_fade_in(),
	      rewind_buffer(), rewind_active(cfg_pulseaudio_seek_fade_out > 0 || cfg_pulseaudio_track_fade_out > 0),
	...
	double get_latency()
	{
		...
		if (m_incoming_spec.is_valid()) {
			ret += audio_math::samples_to_time((m_incoming.get_size() - m_incoming_ptr) / m_incoming_spec.m_channels,
	...
	void process_samples(const audio_chunk &p_chunk)
	{
		...
		if (active_fade_in.active) {
			size_t fade_samples = pfc::min_t(m_incoming.get_size() / m_incoming_spec.m_channels,
			                                 (active_fade_in.total_samples - active_fade_in.progress));
			fade_section(m_incoming.get_ptr(), fade_samples, active_fade_in.total_samples, active_fade_in.progress,
	...
	static GUID g_get_guid()
	{
		static const GUID guid = {
		    0xfe94df9, 0xc8e2, 0x40a1, {0x40, 0xa1, 0xb1, 0x2a, 0x4a, 0x6c, 0xe4, 0x9e}
        };
		return guid;
	}
...
private:
	...
	static void __stdcall sink_input_info_cb(pa_context *c, const pa_sink_input_info *i, int eol, void *userdata)
	{
		output_pulse *output = (output_pulse *)userdata;
		...
		if (g_pa_cvolume_valid(&i->volume) && output->volume != i->volume.values[0]) {
			...
			fb2k::inMainThread([volume_db]() { playback_control::get()->set_volume(volume_db); });
		}
	}
	...
	void fade_section(audio_sample *data, size_t num_samples_to_write, size_t total_fade_samples,
	                  size_t start_at_sample, size_t num_channels, bool fade_in)
	{
		if (fade_in) {
			for (size_t s = 0; s < num_samples_to_write; s++) {
				audio_math::scale(data + (s * num_channels), num_channels, data + (s * num_channels),
				                  (1.0f * (s + start_at_sample)) / (1.0f * total_fade_samples));
			}
		} else {
			for (size_t s = 0; s < num_samples_to_write; s++) {
				audio_math::scale(data + (s * num_channels), num_channels, data + (s * num_channels),
				                  (1.0f * (total_fade_samples - (start_at_sample + s))) / (1.0f * total_fade_samples));
			}
		}
	}
		...
		if (rewind_bytes > 0) {
			...
			int64_t fade_samples =
			    pfc::min_t(rewind_bytes / 4 / m_active_spec.m_channels,
			               (int64_t)m_active_spec.time_to_samples(0.001 * fade_ms) * m_active_spec.m_channels);
			fade_section((audio_sample *)rewind_data.get(), (size_t)fade_samples, (size_t)fade_samples, 0,
			             m_active_spec.m_channels, false);

			int64_t write_bytes = fade_samples * m_active_spec.m_channels * sizeof(audio_sample);
			int error = g_pa_stream_write(stream, (audio_sample *)rewind_data.get(), (size_t)write_bytes, NULL,
