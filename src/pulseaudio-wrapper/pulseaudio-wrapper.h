...
#ifndef DECL_FUNC
#	define DECL_FUNC(type, name, paramswtype, params) WINAPI type WIN_##name paramswtype;
#endif

#ifndef DECL_FUNC_VOID
#	define DECL_FUNC_VOID(name, paramswtype, params) WINAPI void WIN_##name paramswtype;
#endif
...
typedef void(__attribute__((ms_abi)) WINAPI *WIN_pa_context_event_cb_t)(pa_context *c, const char *name, pa_proplist *p,
                                                                        void *userdata);
typedef void(__attribute__((ms_abi)) WINAPI *WIN_pa_context_subscribe_cb_t)(pa_context *c,
                                                                            pa_subscription_event_type_t t,
                                                                            uint32_t idx, void *userdata);
typedef void(__attribute__((ms_abi)) WINAPI *WIN_pa_sink_input_info_cb_t)(pa_context *c, const pa_sink_input_info *i,
...
DECL_FUNC(pa_stream *, pa_stream_new,
          (pa_context * c, const char *name, const pa_sample_spec *ss, const pa_channel_map *map), (c, name, ss, map))
DECL_FUNC(int, pa_stream_connect_playback,
          (pa_stream * s, const char *dev, const pa_buffer_attr *attr, pa_stream_flags_t flags,
...
DECL_FUNC_CB(pa_operation *, pa_stream_drain, (pa_stream * s, WIN_pa_stream_success_cb_t cb, void *userdata),
             (s, cbs, cb_data), void, (pa_stream * c, int success, void *userdata),
             (c, success, cb_data->userdata))
DECL_FUNC_CB_VOID(pa_stream_set_write_callback, (pa_stream * p, WIN_pa_stream_request_cb_t cb, void *userdata),
                  (p, cbs, cb_data), void, (pa_stream * p, size_t nbytes, void *userdata),
                  (p, nbytes, cb_data->userdata))
DECL_FUNC_CB_VOID(pa_stream_set_state_callback, (pa_stream * s, WIN_pa_stream_notify_cb_t cb, void *userdata),
                  (s, cbs, cb_data), void, (pa_stream * p, void *userdata),
                  (p, cb_data->userdata))
DECL_FUNC_CB_VOID(pa_stream_set_started_callback, (pa_stream * p, WIN_pa_stream_notify_cb_t cb, void *userdata),
                  (p, cbs, cb_data), void, (pa_stream * p, void *userdata),
                  (p, cb_data->userdata))
DECL_FUNC_CB_VOID(pa_stream_set_underflow_callback, (pa_stream * p, WIN_pa_stream_notify_cb_t cb, void *userdata),
                  (p, cbs, cb_data), void, (pa_stream * p, void *userdata),
                  (p, cb_data->userdata))
DECL_FUNC_CB(pa_operation *, pa_stream_cork, (pa_stream * s, int b, WIN_pa_stream_success_cb_t cb, void *userdata),
             (s, b, cbs, cb_data), void, (pa_stream * c, int success, void *userdata),
             (c, success, cb_data->userdata))
DECL_FUNC(int, pa_stream_is_corked, (const pa_stream *s), (s))
DECL_FUNC_CB(pa_operation *, pa_stream_flush, (pa_stream * s, WIN_pa_stream_success_cb_t cb, void *userdata),
             (s, cbs, cb_data), void, (pa_stream * c, int success, void *userdata),
             (c, success, cb_data->userdata))
DECL_FUNC_CB(pa_operation *, pa_stream_update_sample_rate,
             (pa_stream * s, uint32_t rate, WIN_pa_stream_success_cb_t cb, void *userdata),
             (s, rate, cbs, cb_data), void, (pa_stream * c, int success, void *userdata),
             (c, success, cb_data->userdata))
...
DECL_FUNC_CB(pa_operation *, pa_stream_trigger, (pa_stream * s, WIN_pa_stream_success_cb_t cb, void *userdata),
             (s, cbs, cb_data), void, (pa_stream * c, int success, void *userdata),
             (c, success, cb_data->userdata))
DECL_FUNC_CB(pa_operation *, pa_stream_update_timing_info,
             (pa_stream * p, WIN_pa_stream_success_cb_t cb, void *userdata),
             (p, pa_stream_update_timing_info_cb, cb_data), void, (pa_stream * c, int success, void *userdata),
             (c, success, cb_data->userdata))
...
/* XXX: This function ALWAYS assumes free_cb is NULL,
 * otherwise there is a memory leak that we can't really do much about :) */
DECL_FUNC(int, pa_stream_write, (pa_stream *p, const void *data, size_t nbytes, pa_free_cb_t free_cb, int64_t offset, pa_seek_mode_t seek), (p, data, nbytes, NULL, offset, seek))
...
DECL_FUNC(int, pa_proplist_set, (pa_proplist * p, const char *key, const void *data, size_t nbytes),
          (p, key, data, nbytes))
DECL_FUNC(int, pa_proplist_setp, (pa_proplist * p, const char *pair), (p, pair))

/* CONTEXT */
DECL_FUNC(pa_context *, pa_context_new_with_proplist,
          (pa_mainloop_api * mainloop, const char *name, const pa_proplist *proplist), (mainloop, name, proplist))
DECL_FUNC_VOID(pa_context_unref, (pa_context * c), (c))
DECL_FUNC(int, pa_context_errno, (const pa_context *c), (c))
DECL_FUNC(int, pa_context_connect,
...
DECL_FUNC_CB_VOID(pa_context_set_state_callback, (pa_context * c, WIN_pa_context_notify_cb_t cb, void *userdata),
                  (c, cbs, cb_data), void, (pa_context * c, void *userdata),
                  (c, cb_data->userdata))
DECL_FUNC_CB_VOID(pa_context_set_event_callback, (pa_context * p, WIN_pa_context_event_cb_t cb, void *userdata),
                  (p, cbs, cb_data), void,
                  (pa_context * c, const char *name, pa_proplist *p, void *userdata), (c, name, p, cb_data->userdata))
DECL_FUNC_CB(pa_operation *, pa_context_get_sink_input_info,
             (pa_context * c, uint32_t idx, WIN_pa_sink_input_info_cb_t cb, void *userdata),
             (c, idx, cbs, cb_data), void,
             (pa_context * c, const pa_sink_input_info *i, int eol, void *userdata), (c, i, eol, cb_data->userdata))
DECL_FUNC_CB(pa_operation *, pa_context_subscribe,
             (pa_context * c, pa_subscription_mask_t m, WIN_pa_context_success_cb_t cb, void *userdata),
             (c, m, cbs, cb_data), void, (pa_context * c, int success, void *userdata),
             (c, success, cb_data->userdata))
DECL_FUNC_CB_VOID(pa_context_set_subscribe_callback, (pa_context * c, WIN_pa_context_subscribe_cb_t cb, void *userdata),
                  (c, cbs, cb_data), void,
                  (pa_context * c, pa_subscription_event_type_t t, uint32_t idx, void *userdata),
                  (c, t, idx, cb_data->userdata))

/* CHANNEL */
DECL_FUNC(pa_channel_map *, pa_channel_map_init_auto, (pa_channel_map * m, unsigned channels, pa_channel_map_def_t def),
          (m, channels, def))
...
DECL_FUNC_CB(pa_operation *, pa_context_set_sink_input_volume,
             (pa_context * c, uint32_t idx, const pa_cvolume *volume, WIN_pa_context_success_cb_t cb, void *userdata),
             (c, idx, volume, cbs, cb_data), void,
             (pa_context * c, int success, void *userdata), (c, success, cb_data->userdata))
